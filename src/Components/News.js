...
function News({setProgress , category}) {
    ...
    const fetchData = async () => {
        setProgress(10)
        const response = await fetch(`https://newsapi.org/v2/top-headlines?country=in&page=${page}&category=${category}&apiKey=1a7a70ac444246afb224ec12c2c5e3cc`)
        setProgress(40)
    ...
    const fetchMoreData = async () => {
        const response = await fetch(`https://newsapi.org/v2/top-headlines?country=in&page=${page + 1}&category=${category}&apiKey=1a7a70ac444246afb224ec12c2c5e3cc`)
        setPage(page + 1)
        const parseData = await response.json()
